import React from "react";
import { AiOutlineCloseCircle, AiOutlineSearch } from "react-icons/ai";
import { CiLocationOn } from "react-icons/ci";
import { popularSearch } from "../../utils/data";
import CustomButton from "../CustomButton";
import { HeroImage } from "../../assets";

const SearchInput = (props) => {
  // handle typing input
  const handleChange = (e) => {
    props.setValue(e.target.value);
  };

  // handle clear input
  const ClearInput = () => props.setValue("");
  return (
    <div className={`flex w-full md:w-1/3 items-center ${props.styles}`}>
      {props.icon}
      <input
        value={props.value}
        onChange={handleChange}
        type="text"
        className="w-full md:w-64 p-2 outline-none bg-transparent text-base"
        placeholder={props.placeholder}
      />
      <AiOutlineCloseCircle
        className="hidden md:flex text-gray-600 text-xl cursor-pointer"
        onClick={ClearInput}
      />
    </div>
  );
};

const Header = (props) => {
  return (
    <div className="bg-[#f7fdfd]">
      <div
        className={`container mx-auto px-5 ${
          props.type ? "h-[500px]" : "h-350px"
        } flex items-center relative`}
      >
        <div className="w-full z-10">
          <div className="mb-8">
            <p className="text-slate-700 font-bold text-4xl">{props.title}</p>
          </div>

          <div className="w-full flex items-center justify-around bg-white px-2 md:px-5 py-2.5 md:py-6 shadow-2xl rounded-full">
            <SearchInput
              placeholder="Job Title or Keywords"
              icon={<AiOutlineSearch className="text-gray-600 text-xl" />}
              value={props.searchQuery}
              setValue={props.setSearchQuery}
            />
            <SearchInput
              placeholder="Add Country or City"
              icon={<CiLocationOn className="text-gray-600 text-xl" />}
              value={props.location}
              setValue={props.setLocation}
              styles={"hidden md:flex"}
            />

            <div>
              <CustomButton
                onClick={props.handleClick}
                title="Search"
                containerStyles={
                  "text-white py-2 md:py-3 px-3 md:px-10 focus:outline-none bg-blue-600 rounded-full md:text-base"
                }
              />
            </div>
          </div>
          {props.type && (
            <div className="w-full lg:1/2 flex flex-wrap gap-3 md:gap-6 py-10 md:py-14">
              {popularSearch.map((item, idx) => {
                return (
                  <span
                    key={idx}
                    className="bg-[#1d4fd826] text-[#1d4ed8] py-1 px-2 rounded-full text-sm md:text-base"
                  >
                    {item}
                  </span>
                );
              })}
            </div>
          )}
        </div>

        <div
          className="w-1/3 h-full absolute top:24 md:-top-6 
                    lg:-top-14 right-16 2xl:right-[18rem]"
        >
          <img src={HeroImage} className="object-contain" />
        </div>
      </div>
    </div>
  );
};

export default Header;
